  ...
  try {
    ...
    if (response.data.success) {
      ...
      return transactions.map((t) => ({
        ...
        completedAt: t.transfer?.completedAt
          ? new Date(t.transfer.completedAt).toLocaleString()
          : "N/A",
        loanId: t.loanId || "N/A",
      }));
    } else {
    ...
    if (error.response?.status === 404) {
      console.warn(
        `No transactions found for account ${accountId} (404 Not Found)`
      );
...
export const fetchTransactionDetails = async (transactionId) => {
  try {
    ...
    if (response.data.success) {
      ...
      return {
        ...
        amount: transaction.amount
          ? `${transaction.amount} ${transaction.currency || "N/A"}`
          : "N/A",
    ...
    if (error.response?.status === 404) {
      console.warn(
        `Transaction details not found (404) for ID: ${transactionId}`
      );
