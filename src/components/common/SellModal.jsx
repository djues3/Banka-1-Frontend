...
function SellModal({ isOpen, onClose, onSave, selectedSecurity }) {
  ...
  const loadAccounts = async () => {
    try {
      ...
      const res = await fetchAvailableAmount(userId, selectedSecurity.securityId);
      ...
      <Dialog open={isOpen} onClose={onClose} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Typography variant="subtitle1" sx={{ marginBottom: 1 }}>
            Available securities: {availableAmount.data}
          </Typography>
          ...
          >
            {accounts
              .filter((acc) => acc.currencyType === "USD")
              .map((option) => (
